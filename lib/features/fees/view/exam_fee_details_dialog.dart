...
class _ExamFeeDetailsDialogState extends State<ExamFeeDetailsDialog>
  ...
  List<ExamPaidFeeModel> _paymentHistory = [];
  ...
  Widget _buildPaymentActions() {
    if (widget.fee is! ExamFeeModel) return const SizedBox.shrink();

    final fee = widget.fee as ExamFeeModel;
    if (fee.remainingAmount <= 0) return const SizedBox.shrink();
  ...
  void _showPaymentDialog(ExamFeeModel fee) {
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  ...
                                  Text(
                                    'Rs. ${fee.totalFee.toStringAsFixed(0)}',
                                    style: GoogleFonts.poppins(
                                      fontSize: 14.sp,
                                      fontWeight: FontWeight.w600,
                                      color: AppColors.primary,
                                ...
                                onPressed: () async {
                                  if (formKey.currentState?.validate() ??
                                    ...
                                    if (paymentAmount > fee.remainingAmount) {
                                      Get.snackbar(
                                        'Invalid Payment',
                                        'Payment amount cannot exceed remaining balance of Rs. ${fee.remainingAmount.toStringAsFixed(0)}',
                                    ...
                                    if (success) {
                                      ...
                                      Get.snackbar(
                                        'Success',
                                        'Exam fee payment processed successfully',
                                        snackPosition: SnackPosition.BOTTOM,
  ...
  Future<void> _loadPaymentHistory() async {
    if (widget.fee is ExamPaidFeeModel) {
      setState(() => _isLoadingHistory = true);
      try {
        // For paid exam fees, get all individual payment entries for this student and exam
        final fee = widget.fee as ExamPaidFeeModel;
        final paidEntries = await ExamFeesService.getExamPaidEntriesByStudent(
          fee.studentId!,
          fee.examName!,
        );

        print('Exam Fee Details → Total Paid Entries: ${paidEntries.length}');
    ...
    } else if (widget.fee is ExamFeeModel) {
      setState(() => _isLoadingHistory = true);
      try {
        // For pending fees, get payment history by exam fee ID
        // This would require a service method similar to monthly fees
        // For now, we'll keep it empty as the current implementation doesn't support partial payments for pending exam fees
        _paymentHistory = [];
      } catch (e) {
        print('Error loading payment history: $e');
      } finally {
